	...
	extends		TMixer
{
	...
	public JEsdMixer()
	{
		...
		{
			TDebug.out("JEsdMixer.<init>: beginning.");
		}
		if (TDebug.TraceMixer)
		{
			TDebug.out("JEsdMixer.<init>: end.");
		}
	}
		...
		throws	LineUnavailableException
	{
		if (TDebug.TraceMixer)
		{
			TDebug.out("JEsdMixer.getSourceDataLine(): format: " + format);
			TDebug.out("JEsdMixer.getSourceDataLine(): buffer size: " + nBufferSize);
		}
		if (nBufferSize < 1)
		{
		...
		JEsdSourceDataLine	sourceDataLine = new JEsdSourceDataLine(this, format, nBufferSize);
		// registerChannel(sourceDataLine);
		sourceDataLine.start();
		if (TDebug.TraceMixer)
		{
			TDebug.out("JEsdMixer.getSourceDataLine(): returning: " + sourceDataLine);
		}
	...
	{
		int			nBufferSizeInBytes = nBufferSize * format.getFrameSize();
		JEsdTargetDataLine	targetDataLine = new JEsdTargetDataLine(this, format, nBufferSizeInBytes);
		// registerChannel(sourceDataLine);
		targetDataLine.start();
		if (TDebug.TraceMixer)
		{
			TDebug.out("JEsdMixer.getTargetDataLine(): returning: " + targetDataLine);
		}
